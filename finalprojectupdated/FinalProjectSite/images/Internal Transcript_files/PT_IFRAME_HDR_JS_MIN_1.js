...
var ptBaseURI = getptBaseURI();var ptIframeHdr = {
 ...
 isSmartNav : false,
 noDescrVal : false, 
 searchURI : ptBaseURI + "s/WEBLIB_PORTAL.PORTAL_SEARCH_PB.FieldFormula.IScript_SESPortalQry",
 ...
 if (typeof(linkObj.href) !== "undefined") { open(linkObj.href, "_parent"); } 
 }
 } else { 
 if (typeof(linkObj.href) !== "undefined") { open(linkObj.href, "_parent"); }
 }
...
var ptPopup = function (el,options) {
 ...
 var popupWidth; var c = this.container; if (this.quirksMode) {

 
 
 var cWidth = c.offsetWidth - c.clientWidth;  if (c.currentStyle !== 'undefined' && c.currentStyle) {
 cWidth += parseFloat(c.currentStyle["paddingLeft"]) + parseFloat(c.currentStyle["paddingRight"]); cWidth += parseFloat(c.currentStyle["marginLeft"]) + parseFloat(c.currentStyle["marginRight"]); }
 popupWidth = this.layoutTbl.offsetWidth + cWidth;    } else if (!document.documentMode || document.documentMode === 7) {
 if (c.currentStyle !== 'undefined' && c.currentStyle) {
 ...
 popup.msgTitle.firstChild);    var res = msg.split(/\s*<br ?\/?>\s*/i);  var msgNode = document.createElement("P"); if (res[0].length>0) { 
 msgNode.appendChild(document.createTextNode(res[0])); }

 var i;  for (i = 1; i < res.length; i++) {
 msgNode.appendChild(document.createElement('BR')); if (res[i].length > 0) {
 msgNode.appendChild(document.createTextNode(res[i])); }
 } 

 popup.msgText.replaceChild(msgNode,popup.msgText.firstChild);   popup.msgBtn2.parentNode.parentNode.style.display = "inline-block"; this.show(); this.btnFocus.focus(); }
 },
